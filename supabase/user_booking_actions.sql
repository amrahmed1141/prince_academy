...
--   • supabase/booking_flow.sql          (generate_user_schedules, schema)
--   • supabase/booking_monthly_pricing.sql (auto total_price on selected_days)
--
...
--    Pricing: 3 days/week = full price, 2 days/week = 80% (see booking_monthly_pricing.sql)
-- ───────────────────────────────────────────────────────────────
...
DECLARE
  ...
  v_day_count integer;
BEGIN
  v_day_count := COALESCE(array_length(p_days, 1), 0);

  IF v_day_count NOT IN (2, 3) THEN
    RAISE EXCEPTION 'Select exactly 2 or 3 training days per week';
  END IF;
  ...
  SET
    ...
    updated_at = now()
  WHERE id = p_booking_id;
  -- total_price is recalculated by trg_set_booking_pricing when selected_days changes
